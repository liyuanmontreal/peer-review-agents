...
`verify_all_claims()` is the batch entry point. It:
1. Loads all verifiable claims for a paper from the `claims` table
2. Filters already-verified claims (unless `force=True`)
3. Runs `verify_claim()` per claim in a `ThreadPoolExecutor` (default `max_workers=2`)
4. After each result, runs span alignment and appends `aligned_span_ids`
5. Persists all results via `save_verification_results()`
6. Emits `GSR_PROGRESS verify_claims {completed} {total}` to stdout after each claim

**📍 Code Pointers:**
- `src/gsr/claim_verification/verifier.py` → `verify_all_claims()`
- `src/gsr/claim_verification/__init__.py` → public API exports
- Progress signal: look for `GSR_PROGRESS verify_claims` in `verifier.py`

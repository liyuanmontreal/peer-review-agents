...
DEFAULT_INITIAL_PROMPT = (
    ...
    "Before posting:\n"
    "  a. Write the required artifact files under `reviews/<paper_id>/` using these exact names:\n"
    "     - First time on a paper: `paper_<paper_id>_summary.md`\n"
    "     - Before a comment: `comment_draft_<paper_id>_<shortid>.md` and `comment_trace_<paper_id>_<shortid>.json`\n"
    "     - Before a verdict: `verdict_draft_<paper_id>_<shortid>.md` and `verdict_trace_<paper_id>_<shortid>.json`\n"
    "     Use a timestamp (e.g. `20260425T120000Z`) as `<shortid>`. "
    "Never create files named `review_<paper_id>_<timestamp>.md`.\n"
    "  b. Commit and push the artifact files to your agent's GitHub repo.\n"
    "  c. Construct the GitHub URL for the draft file "
